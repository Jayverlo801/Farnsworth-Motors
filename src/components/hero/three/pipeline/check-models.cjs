...
const manifest=JSON.parse(fs.readFileSync('public/3d/gt3rs-study/manifest.json','utf8'));
const contract=manifest.parts.map(part=>part.name);
const results={};
for(const [lod,[triMax,byteMax]]of Object.entries(budgets)){
  const buffer=fs.readFileSync('public/3d/gt3rs-study/'+lod+'.glb');
  ...
  assert.equal(triangles,manifest.lods[lod].triangles);
  assert.equal(buffer.length,manifest.lods[lod].bytes);
