...
OUT=PIPE/'verification/gt3rs-refinement' if '--preview' in sys.argv else PIPE.parents[4]/'assets/3d/source/gt3rs-study'
...
floor=bpy.data.objects['STUDIO / floor'];cove=bpy.data.objects.get('STUDIO / cove')
floor.data.materials.clear();floor.data.materials.append(bpy.data.materials['studio floor'])
try:
...
selected=[a for a in sys.argv[sys.argv.index('--')+1:] if not a.startswith('--')] if '--' in sys.argv else []
for name,camera,frame in [('hero','hero',270),('rear','rear',270),('side','side',270),('exploded','hero',1)]:
    if selected and name not in selected:continue
    floor.hide_render=name=='side'
    if cove:cove.hide_render=name=='side'
...
if cove:cove.hide_render=False
